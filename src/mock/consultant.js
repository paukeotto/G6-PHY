...
// Mock API - 获取咨询师列表
Mock.mock(/\/api\/consultants\/list(\?.*)?$/, 'get', (options) => {
  console.log('Mock intercepted:', options.url)
  ...
  if (keyword) {
    ...
    filteredConsultants = consultants.filter(consultant => 
      consultant.username.toLowerCase().includes(lowercaseKeyword) ||
      consultant.profile.toLowerCase().includes(lowercaseKeyword) ||
      consultant.tags.some(tag => tag.toLowerCase().includes(lowercaseKeyword))
    )
  }
  ...
  const start = (page - 1) * pageSize
  const end = start + pageSize
  const paginatedData = filteredConsultants.slice(start, end)
